...
class SgdTrainer:
    def __init__(
        ...
        self.train_dataset = self.data_container.get_dataset(
    ...
    def input_shape(self):
        ...
        dataloader = DataLoader(
            dataset=dataset, batch_size=1, collate_fn=dataset.collator)
    ...
    def get_default_callbacks(self):
        ...
        if not self.update_counter.is_finished:
            ...
            default_callbacks += [
                ...
                OnlineLossCallback(**default_kwargs, **
                                   default_intervals, verbose=True),
            ]

            # EtaCallback is pointless in managed runs
            # - managed runs don't have an interactive console
            if not is_managed() and is_rank0():
                default_callbacks = [EtaCallback(
                    **default_kwargs, **default_intervals)] + default_callbacks
    ...
    def _calculate_batch_size_and_accumulation_steps(self, model, ddp_model):
        ...
        effective_batch_size_per_device = int(
            self.effective_batch_size / world_size)
        ...
        if model.is_batch_size_dependent:
            if self.force_gradient_accumulation:
                self.logger.info(
                    "model is batch_size dependent but gradient accumulation is forced")
                assert not self.disable_gradient_accumulation
            else:
                self.logger.info(
                    "model is batch_size dependent -> disabled gradient accumulation")
                return effective_batch_size_per_device, 1
        ...
        if get_num_nodes() > 1 and self.max_batch_size is None:
            self.logger.info(
                f"found multi-node setting -> disable automatic batchsize (occasionally hangs)")
            return effective_batch_size_per_device, 1
        if self.use_torch_compile and self.max_batch_size is None:
            self.logger.info(
                "torch.compile is used -> automatic batchsize not supported")
            return effective_batch_size_per_device, 1
        ...
        if effective_batch_size_per_device <= max_batch_size:
            ...
            accumulation_steps = int(
                effective_batch_size_per_device / max_batch_size)
            batch_size = int(
                effective_batch_size_per_device / accumulation_steps)
        self.logger.info(f"batch_size: {batch_size}")
        self.logger.info(f"accumulation_steps: {accumulation_steps}")
        return batch_size, accumulation_steps

    def state_dict(self):
        callback_state_dicts = [callback.state_dict()
    ...
    def wrap_ddp(self, trainer_model):
        if is_distributed():
            ...
            if trainer_model.device != torch.device("cpu") and self.sync_batchnorm:
                self.logger.info(
                    f"replacing BatchNorm layers with SyncBatchNorm")
                trainer_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    trainer_model)
    ...
    def wrap_compile(self, ddp_model):
        if not self.use_torch_compile or os.name == "nt":
            self.logger.info(
                f"torch.compile not used (use_torch_compile == False)")
            return ddp_model
        if is_distributed():
            if self.static_graph:
                self.logger.info(
    ...
    def train(self, model, callbacks=None):
        ...
        self.call_after_training(
            trainer_model=trainer_model, callbacks=callbacks)
    ...
    def _train(
        ...
        if self.update_counter.is_finished:
            ...
            while True:
                ...
                while True:
                    # check end of epoch
                    remaining_batches = train_batches_per_epoch - \
                        (iter_step + 1)
                    if remaining_batches < accumulation_steps:
                        ...
                        for _ in range(remaining_batches):
                            _ = next(data_iter)
                        break
                    is_last_update_in_epoch = remaining_batches - \
                        accumulation_steps < accumulation_steps
                    for callback in periodic_callbacks:
                    ...
                    for _ in range(accumulation_steps):
                        ...
                        for callback in periodic_callbacks:
                            callback.before_every_accumulation_step(
        ...
        if inspect.isgeneratorfunction(ddp_model.forward):
            ...
            while True:
                try:
                    ...
                    break
            all_losses["total"] = sum(
                [v for k, v in all_losses.items() if k.startswith("total.")])
        else:
    ...
    def _step(
        ...
        if not model.is_frozen:
            ...
            if is_first_update:
                ...
                self.logger.info(
                    f"{len(unused_param_names)} unused parameters")
            ...
            if (iter_step + 1) % accumulation_steps == 0:
                ...
                if self.skip_nan_loss:
                    ...
                    if torch.any(torch.isnan(total_loss)):
                        self.logger.info(
                            f"encountered nan loss -> skip (counter: {self.skip_nan_loss_counter})")
                        do_step = False
                        self.skip_nan_loss_counter += 1
                        if self.skip_nan_loss_counter > 100:
                            raise RuntimeError(
                                f"encountered 100 nan losses in a row")
                    else:
                        # reset counter
                        if self.skip_nan_loss_counter > 0:
                            self.logger.info(
                                f"encountered valid loss after {self.skip_nan_loss_counter} nan losses")
                            self.skip_nan_loss_counter = 0
    ...
    def call_after_training(self, trainer_model, callbacks):
        ...
        for callback in callbacks:
            callback.after_training(
                model=trainer_model.model, trainer=self, update_counter=self.update_counter)
        CallbackBase.flush()
