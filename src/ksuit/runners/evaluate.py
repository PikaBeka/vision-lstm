...
class Runner_evaluate():
    ...
    def main(device):
        ...
        # Configurable knobs (can also come from your YAML/CLI if you prefer)
        bench_bs = int(stage_hp.get("bench_bs", 1))        # batch size
        warmup = int(stage_hp.get("bench_warmup", 50))    # warmup iterations
        iters = int(stage_hp.get("bench_iters", 300))    # timed iterations
        use_amp = bool(stage_hp.get("bench_amp", True)
                       )    # BF16/FP16 if available

        # Random input matching the model’s expected input
        x = torch.randn(bench_bs, C, H, W, device=device)

        # Some models in this codebase return dicts with key "main"
        # We'll handle both tensor and dict outputs safely. :contentReference[oaicite:0]{index=0}
        def _forward(m, inp):
            out = m(inp)
            if isinstance(out, dict):
                return out.get("main", next(iter(out.values())))
            return out

        # Warmup: let cuDNN autotune settle & JIT paths stabilize
        ...
        # Timed runs with CUDA events for accurate wall-time on GPU
        ...
        total_ms = start.elapsed_time(end)  # milliseconds for all iters
        avg_ms_per_iter = total_ms / iters
        avg_ms_per_image = avg_ms_per_iter / bench_bs
        imgs_per_sec = 1000.0 / avg_ms_per_image

        logging.info("========== Inference benchmark ==========")
        logging.info(f"Device: {device}, AMP: {use_amp}")
        logging.info(f"Input: BxCxHxW = {bench_bs}x{C}x{H}x{W}")
        logging.info(f"Warmup: {warmup}, Iters: {iters}")
        logging.info(f"Avg latency/iter: {avg_ms_per_iter:.3f} ms")
        logging.info(f"Avg latency/image: {avg_ms_per_image:.3f} ms")
        logging.info(f"Throughput: {imgs_per_sec:.2f} img/s")
        logging.info("=========================================")

        # Prevent the training pipeline from running
        return
