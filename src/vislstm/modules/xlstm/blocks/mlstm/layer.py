...
class mLSTMLayer(nn.Module):
    ...
    def __init__(self, config: mLSTMLayerConfig):
        ...
        branch_dim = self.config.embedding_dim // 2
        ...
        if self.config.layerscale is None:
            ...
            self.layerscale = LayerScale(
                dim=self.config.embedding_dim, init_scale=self.config.layerscale)
    ...
    def forward(self, x: torch.Tensor, block_idx=None, **kwargs) -> torch.Tensor:
        ...
        if self.config.alternation is None:
            ...
            if block_idx % 4 == 1:
                ...
                z = z.flip(dims=[1])
            if block_idx % 4 == 2:
                x_mlstm = einops.rearrange(
                    x_mlstm, "b (h w) d -> b (w h) d", w=w)
                z = einops.rearrange(z, "b (h w) d -> b (w h) d", w=w)
            if block_idx % 4 == 3:
                x_mlstm = einops.rearrange(
                    x_mlstm, "b (h w) d -> b (w h) d", w=w).flip(dims=[1])
                z = einops.rearrange(
        ...
        if self.config.alternation is None:
            ...
            if block_idx % 4 == 1:
                h_state = h_state.flip(dims=[1])
            if block_idx % 4 == 2:
                h_state = einops.rearrange(
                    h_state, "b (w h) d -> b (h w) d", w=w)
            if block_idx % 4 == 3:
                h_state = einops.rearrange(
                    h_state, "b (w h) d -> b (h w) d", w=w).flip(dims=[1])
            else:
                pass
        else:
            raise NotImplementedError
        ...
        if self.config.bidirectional or self.config.quaddirectional:
            ...
            h_tilde_state_skip_rev = h_tilde_state_rev + \
                (learnable_skip_rev * x_mlstm_conv_act_rev)

            h_state = h_state + (h_tilde_state_skip_rev *
                                 self.ogate_act_fn(z_rev)).flip(dims=[1])
        ...
        if self.config.quaddirectional:
            ...
            out_du = einops.rearrange(
                h_tilde_state_skip_du * self.ogate_act_fn(z_du), "b (w h) d -> b (h w) d", w=w)
            h_state = h_state + out_du

            x_mlstm_ud = einops.rearrange(
                x_mlstm, "b (h w) d -> b (w h) d", w=w).flip(dims=[1])
            z_ud = einops.rearrange(
                z, "b (h w) d -> b (w h) d", w=w).flip(dims=[1])
            ...
            out_ud = einops.rearrange(
                h_tilde_state_skip_ud * self.ogate_act_fn(z_ud), "b (w h) d -> b (h w) d", w=w)
