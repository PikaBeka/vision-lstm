...
for e in range(epochs):
    ...
    for x, y in train_dataloader:
        ...
        if np.random.rand() < cutmix_prob:
            ...
            loss = lam * F.cross_entropy(outputs, y_a, label_smoothing=0.1) + (1 - lam) * F.cross_entropy(outputs, y_b, label_smoothing=0.1)
        else:
            outputs = model(x)
        ...
        # status update
        train_accuracy = ((outputs.argmax(dim=1) == y).sum() / y.numel()).item()
    ...
    if (e + 1) % 5 == 0 or e == 0 or e == epochs - 1:  # Evaluate every 5 epochs to save time
        ...
        if (e + 1) % 10 == 0 or e == epochs - 1:  
            ...
            plt.plot(epochs_completed, [acc * 100 for acc in epoch_train_accs], label='Train Accuracy')
            plt.plot(range(len(epoch_test_accs)), [acc * 100 for acc in epoch_test_accs], label='Test Accuracy')
            ...
            plt.suptitle(f'{MODEL_NAME} Training on CIFAR-100 - Epoch {e+1}/{epochs}', fontsize=16)
            plt.tight_layout(rect=[0, 0, 1, 0.97])
            
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plt.savefig(f"{plot_dir}/training_curves_epoch{e+1}_{timestamp}.png", dpi=300)
...
plt.subplot(2, 2, 2)
plt.plot(range(len(epoch_train_accs)), [acc * 100 for acc in epoch_train_accs], label='Train Accuracy')
plt.plot(range(len(epoch_test_accs)), [acc * 100 for acc in epoch_test_accs], label='Test Accuracy')
plt.title(f'{MODEL_NAME} - Accuracy per Epoch')
plt.xlabel('Epoch')
plt.ylabel('Accuracy (%)')
